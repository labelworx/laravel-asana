...
class Asana
{
    ...
    public function applyData($data)
    {
        ...
        array_walk_recursive($this->templateData, function(&$dataValue, $key, $data) {
            $dataValue = str_replace(array_keys($data), array_values($data), $dataValue );
        }, $data);
    ...
    public function getTemplate($className)
    {
        if (!class_exists($className)) {
            throw new \Exception('Asana Template Class does not exist');
        }
        $this->templateData = (new $className)->buildTaskData();
        return $this;
    }
